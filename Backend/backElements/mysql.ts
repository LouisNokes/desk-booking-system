...
async function sqlgetAllDesks(site:String)
{
  let ans = await connection.promise().query('SELECT * FROM desks INNER JOIN sites ON desks.site_idA = sites.site_id WHERE sites.site = "' +site+'"' )
  return ans;
 }
//sqlgetAllDesks('Gloucester');

async function bookings(site:String)
{
  let ans = await connection.promise().query('SELECT * FROM desk_bookings' );
  return ans;
 }
...
async function sqlgetAllBookings(site:String,desk:Number, dateto:Date,datefrom:Date )
...
//ADD A BOOKING.............................
function sqlAddBooking(book_site:string,book_desk:Number,book_userid:Number,book_datefromstr:String,book_datetostr:String)
{
  //if(sqlgetAllBookings('Manchester',12, new Date('2022-10-28'),new Date('2022-10-26')) != null)
//{return null}
console.log(book_datetostr);
const [tmonth, tday, tyear] = book_datefromstr.split('/');
let book_datefrom = new Date(+tyear, +tmonth - 1, +tday);

const [fmonth, fday, fyear] = book_datetostr.split('/');
let book_dateto = new Date(+fyear, +fmonth - 1, +fday);


let post = { booking_desk:book_desk ,booking_site:12, booking_datefrom:book_datefrom, booking_dateto:book_dateto,booking_userid:book_userid };
...
module.exports.book = sqlAddBooking;
